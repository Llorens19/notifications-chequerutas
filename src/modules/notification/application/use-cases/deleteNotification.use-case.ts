...
export const deleteNotificationUseCase = async ({ repo, request }: IUseCaseData<INotificationOutputPort>): Promise<IResp<INotification>> => {

  const { idNotification } = request.params as {idNotification: string};

  const response = await repo.deleteNotification(idNotification);
